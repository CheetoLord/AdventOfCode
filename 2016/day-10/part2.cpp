...
void doExchange(string instr, int *chip1, int *chip2, int *outputs) {
    if(instr.substr(0, 5) == "value") {
        return;
    }
...
int main() {
    ...
    while(true) {
        while(getline(input, instr)) {
            doExchange(instr, storedChip1, storedChip2, outputBins);
        }
        input.clear();
        input.seekg(0);
        if(targetBotNum != -1) {
    ...
    cout << "the product of the chip values in output bions 1, 2, and 3 is: " << (outputBins[0]*outputBins[1]*outputBins[2]) << endl;
